...
const collections = resolve(process.cwd(), './collections')
...
function getAllCollectionSlugs(collection) {
	...
	return readdirSync(resolve(collections, collection))
...
function getPostBySlug(slug, collection, inline = false) {
	...
	const file = readFileSync(
		resolve(collections, collection, `${slug}.md`),
		'utf-8'
	)
	...
	return {
		...
		documents: {
			...
			slides: existsSync(resolve(collections, 'slides', `${slug}.md`))
...
const unifiedMarkdownToHtml = content =>
	unified()
		...
		.use(remark2rehype)
		.use(katex)
		.use(stringify) //TODO: use rehype-wrap to wrap these in html class .article
		.use(highlight, {
			languages: new Map(
