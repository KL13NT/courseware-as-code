...
describe('htmlToPdf', () => {
	...
	it('should return a buffer', async () => {
		buffer = await htmlToPdf({ page, html: '<h1/>' })
		expect(buffer).toBeInstanceOf(Buffer)
	})

	it('should throw', async () => {
		await expect(
			htmlToPdf({ page, html: '<h1/>', stylePaths: '../styling/layout.css' })
		).rejects.toThrow()
	})
	...
	it('should return a different buffer when using styles', async () => {
		expect(
			await htmlToPdf({
				page,
				html: '<h1/>',
				stylePaths: ['../styling/layout.css'],
			})
		).not.toEqual(Buffer)
	})

	it('should return a different buffer when using styleContents as well as stylePaths', async () => {
		expect(
			await htmlToPdf({
				page,
				html: '<h1/>',
				stylePaths: ['../styling/layout.css'],
				styleContents: '* {color: red;}',
			})
		).not.toEqual(Buffer)
	})
})
